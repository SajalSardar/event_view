    ...
    <form wire:submit="update" method="POST">
        <div class="grid md:grid-cols-2 sm:grid-cols-1 sm:gap-1 md:gap-4">
            <div class="p-2 w-full">
                ...
                <x-forms.text-input type="text" value="{{ $menu->name }}" wire:model.live="name" placeholder="Menu name" />
            ...
            <div class="p-2 w-full">
                ...
                <x-forms.select-input wire:model="parent_id">
                    ...
                    @foreach ($parent_menus as $parent_menu)
                        <option @selected(old('parent_id', $menu?->parent_id) == $parent_menu?->id) value="{{ $parent_menu->id }}">{{ $parent_menu->name }}</option>
                    @endforeach
        ...
        <div class="grid md:grid-cols-2 sm:grid-cols-1 sm:gap-1 md:gap-4">
            <div class="p-2 w-full">
                ...
                <x-forms.text-input type="text" value="{{ $menu->route }}" wire:model.live="route" placeholder="Route name" />
            ...
            <div class="p-2 w-full">
                ...
                <x-forms.text-input type="text" value="{{ $menu->url }}" wire:model.live="url" placeholder="Full url" />
            ...
            <div class="p-2 w-full">
                ...
                <x-forms.nice-select wire:model="role" multiple>
                    ...
                    @foreach ($roles as $role)
                        <option {{ in_array($role->name, json_decode($menu->roles)) ? 'selected' : '' }} value="{{ $role->name }}">{{ Str::ucfirst($role->name) }}</option>
                    @endforeach
                </x-forms.nice-select>
            </div>
