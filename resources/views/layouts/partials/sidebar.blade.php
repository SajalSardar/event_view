    ...
    <ul class="mt-4 bg-primary-500 h-full">
        <li class="group py-4 px-5 relative">
            <a href="{{ route('dashboard') }}"
                    class="flex font-semibold items-center text-gray-900 rounded-md group-[.active]:bg-gray-800 group-[.active]:text-white group-[.selected]:bg-gray-950 group-[.selected]:text-gray-100 {{ Route::is('dashboard') ? 'before:absolute before:rounded-l-xl before:content-[""] before:w-[4px] before:h-full before:bg-primary-400 before:top-0 before:right-0' : '' }}">
            <span
                ...
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="text-base font-inter font-semibold ml-2">Dashboard</span>
                </span>
            </a>
        </li>
        @foreach (Helper::getAllMenus() as $menu)
            ...
            <li class="group py-4 px-5 relative {{ Route::is($sbMenu) ? 'selected' : '' }} ">
                <a href="{{ $menu->route == '#' ? '#' : ($menu->url ? url($menu->url) : route($menu->route)) }}"
                    class="block font-semibold items-center text-gray-900 rounded-md before:absolute before:rounded-l-xl before:content-[''] before:w-[4px] before:h-[55px] before:top-0 before:right-0  {{ Route::is($menu->route) || Route::is($sbMenu) || url($menu->url) == Request::fullUrl() ? 'before:bg-primary-400' : '' }} {{ count($menu->submneus) > 0 ? 'sidebar-dropdown-toggle' : '' }}">
                    <span class="flex items-center {{ Route::is($menu->route) || Route::is($sbMenu) || url($menu->url) == Request::fullUrl() ? 'text-[#333]' : 'text-[#5c5c5c]'}}">
                     <span>{!! $menu->icon !!}</span>
                        <i class='bx bx-user mr-3 text-lg'></i>
                        <span class="text-base font-inter font-semibold">{{ $menu->name }}</span>
