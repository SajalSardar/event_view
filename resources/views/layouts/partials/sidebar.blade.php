<div class="fixed left-0 top-0 w-64 h-full bg-white py-10 z-50 sidebar-menu transition-transform" style="border-right: 1px solid #cfcece">
    ...
    <ul class="mt-3 bg-white h-full">
        <li class="group pl-6 relative">
            <a href="{{ route('dashboard') }}" class="flex py-4 items-center  {{ Route::is('dashboard') ? 'bg-primary-600' : '' }}">
                <span class="pl-4 flex items-center">
                    ...
                    <span class="ml-2 {{ Route::is('dashboard') ? 'text-dashboard-active' : 'text-dashboard-inactive'}}">Dashboard</span>
                </span>
            </a>
        </li>
        @foreach (Helper::getAllMenus() as $menu)
                ...
                <li class="group pl-6 relative {{ Route::is($sbMenu) ? 'selected' : '' }} ">
                    @if ($menu->route !== 'logout')
                        <a href=" {{ $menu->route == '#' ? '#' : ($menu->url ? url($menu->url) : route($menu->route)) }}" class="block py-4 text-sm font-inter text-[#333] hover:bg-primary-600 {{ Route::is($menu->route) || Route::is($sbMenu) || url($menu->url) == Request::fullUrl() ? '!font-semibold bg-primary-600' : '' }} {{ count($menu->submneus) > 0 ? 'sidebar-dropdown-toggle' : '' }}">
                            <span class="flex items-center">
                                <span class="pl-4">{!! $menu->icon !!}</span>
                                <span class="ml-2 {{ Route::is($menu->route) || Route::is($sbMenu) || url($menu->url) == Request::fullUrl() ? 'text-dashboard-active' : 'text-dashboard-inactive'}}">{{ $menu->name }}</span>
                    ...
                    @if ($menu->route == 'logout')
                        <form action="{{ route('logout') }}" method="POST" class="block py-4 hover:bg-primary-600 {{ Route::is($menu->route) || Route::is($sbMenu) || url($menu->url) == Request::fullUrl() ? 'bg-primary-600' : '' }} {{ count($menu->submneus) > 0 ? 'sidebar-dropdown-toggle' : '' }} ">
                            @csrf
                    ...
                    @if (count($menu->submneus) > 0)
                        <ul class="ml-3 hidden group-[.selected]:block">
                            @foreach ($menu->submneus as $submenu)
                                <li class="relative">
                                    <a href="{{ $submenu->route == '#' && $submenu->url ? url($submenu->url) : route($submenu->route) }}" class="pl-2 px-2 py-4 flex items-center block hover:bg-primary-600  {{ Route::is($submenu->route) || url($submenu->url) == Request::fullUrl() ? 'bg-primary-600' : '' }}">
                                        <span class="flex items-center {{ Route::is($submenu->route) || url($submenu->url) == Request::fullUrl() ? 'text-dashboard-active' : 'text-dashboard-inactive' }}">
